...
// Determine sessions directory based on environment
function getSessionsDir() {
  // If SESSIONS_DIR is set via environment variable (from Electron), use it
  if (process.env.SESSIONS_DIR) {
    return process.env.SESSIONS_DIR;
  }
  
  // For Electron production: use app.getPath('userData') if available
  // This will be set via environment variable from Electron main process
  // For development or non-Electron: use relative path
